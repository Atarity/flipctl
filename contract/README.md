...
- **Кодогенерация типов**:
  - Go (`flipctld`): `oapi-codegen` из `openapi.yaml` для серверных типов запросов/ответов.
  - TypeScript (Web, `frontend.md §5`): `openapi-typescript` из `openapi.yaml`.
  - Rust (TUI, `frontend.md §6`): `progenitor` или `openapi-generator` (Rust-клиент) из `openapi.yaml`.
  - Для `ipc-messages.schema.json` (потребляется только внутри плагинов и `flipctld`, не фронтендами) — `quicktype` умеет генерировать типы под большинство языков плагинов из одной JSON Schema, если понадобится.
- Это ровно тот путь, который `backend.md §11.1` держит про запас на случай, если ручная синхронизация Go (backend) / Rust (TUI) / TypeScript (Web) станет проблемой — сейчас реализуется только сама схема, кодогенерация — по необходимости при реализации.
