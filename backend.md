...
И Web (`fetch`/`EventSource`), и TUI (`net/http` + построчный SSE-парсинг) обращаются к этому API одинаково — ровно так, как уже спроектировано в `frontend.md` разделах 5 и 6.4. TUI, будучи на одном языке с `flipctld`, использует для request/response-типов общий сгенерированный Go-пакет (`contract/go/apitypes`, `frontend.md §9`), а не ручные структуры.
...
**Решение: Go — основной инструмент для `flipctld`.** Таблица ниже сохраняется не как открытый вопрос, а как зафиксированный повод для пересмотра в будущем — если появится конкретная причина, к сравнению можно будет вернуться предметно, а не абстрактно.
...
| Синергия с TUI-фронтендом (`frontend.md §6`) | **Достигнута** — TUI переписан на Go (`bubbletea`), общий сгенерированный Go-пакет для api-contract типов (`contract/go/apitypes`, раздел 11.1) | Требовала бы держать TUI на Rust — решение пересмотрено именно ради этой синергии |
| Резидентный footprint на RPi4 | Малый (статический бинарник, GC чуть тяжелее Rust, но заметно легче Node) | Минимальный |

Изначально TUI был на Rust (`ratatui`) и языки не совпадали — синергия с backend была задокументирована только как гипотетический бонус варианта "Rust для обоих" (раздел 11.1 ниже описывает, как именно решение было пересмотрено). Сейчас TUI переписан на Go специально ради этой синергии — она больше не гипотетическая.
...
### 11.1 Синергия TUI↔backend достигнута: оба на Go

Исходное решение (см. историю этого документа) было принять разные языки как есть: backend на Go, TUI на Rust (`ratatui`) — с ручной синхронизацией `api-contract` в обе стороны и языконезависимой схемой (JSON Schema/OpenAPI) как дешёвым путём отступления на случай, если ручная синхронизация станет реальной болью. Решение пересмотрено: **TUI переписан на Go (`bubbletea`/`lipgloss`/`bubbles`, `frontend.md §6`)** именно ради этой синергии.

Механизм — не "языконезависимая схема, из которой генерируются структуры на разных языках" (это по-прежнему сценарий для пары Go↔TypeScript, Web не становится Go), а прямое переиспользование: `contract/openapi.yaml` прогоняется через `oapi-codegen` **один раз**, результат — Go-пакет `contract/go/apitypes` (`frontend.md §9`), который и `flipctld`, и `flipctl-tui` импортируют как обычную зависимость. Никакой ручной синхронизации структур между backend и TUI не требуется — это одна кодовая база на уровне типов, а не два независимых генератора, синхронизированных по одной схеме.

Чем это отличается от полного слияния в один Go-модуль/монорепо (тоже рассматривалось): `flipctld` и `flipctl-tui` остаются отдельными Go-модулями в отдельных репозиториях/директориях (соответствующих разделению backend.md/frontend.md), связанными только через `contract/go/apitypes` как обычную зависимость — не через прямые импорты internal-пакетов друг друга. Это сохраняет чистое разделение "что решает backend" / "что решает frontend", которое документы поддерживали всю дорогу, при этом полностью закрывая цель синергии.
